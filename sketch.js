...
function solver(){
  ...
  pos[0][0]=1;
  console.log(pos);
  solve_func(maze,0,0, pos, solve_path);
}

 

function solve_func(maze, i, j, pos, solve_path){
  ...
  this.i=i;
  this.j=j;
  console.log("current position"+this.i+" "+ this.j);
  this.visited=true;
  if(i==maze.length-1 && j==maze[0].length-1){
    this.visited=false;
    // path_show(visited);
    console.log(solve_path);
    console.log("hurray");
    return;
  }
      ...
      if(maze[i][j][k]==false){
        ...
        if(k==0){   //top
          if(pos[i-1][j]===0){
            pos[i-1][j]=1;
            solve_path[i-1][j]=1;
            solve_func(maze, i-1, j, pos, solve_path);
            pos[i-1][j]=0;
          }
        }
        if(k==1){   // right
          if(pos[i][j+1]===0){
            pos[i][j+1]=1;
            solve_path[i][j+1]=1;
            solve_func(maze, i, j+1, pos, solve_path);
            pos[i][j+1]=0;
          }
        }
        if(k==2){   //bottom
